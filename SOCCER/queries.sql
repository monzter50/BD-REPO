...
SELECT JORNADA.IDJORNADA,VISITAS.NOMBRE AS VISITAS, 
	LOCAL.NOMBRE AS LOCALES, CANCHA.NOMBRE, 
	JORNADA.FECHA, JORNADA.GOLESVISITAS, 
	JORNADA.GOLESLOCAL,
	CASE 
	 	WHEN (JORNADA.GOLESVISITAS < JORNADA.GOLESLOCAL) THEN LOCAL.NOMBRE
		WHEN (JORNADA.GOLESLOCAL < JORNADA.GOLESVISITAS) THEN VISITAS.NOMBRE
		ELSE 'EMPATE'
		END AS WINNER
	FROM JORNADA 
	INNER JOIN EQUIPOS AS VISITAS ON VISITAS.ID = JORNADA.IDVISITAS 
	INNER JOIN EQUIPOS AS LOCAL ON LOCAL.ID = JORNADA.IDLOCAL 
	INNER JOIN CANCHA ON CANCHA.ID = JORNADA.IDCANCHA;


    ---- TABLERO

    CREATE TEMP TABLE TABLERO (
...
SELECT JORNADA.IDJORNADA,VISITAS.NOMBRE AS VISITAS, 
	...
	CASE 
		...
		END AS WINNER,
	CASE 
	 	WHEN LOCAL.ID = JORNADA.IDLOCAL AND (JORNADA.GOLESVISITAS < JORNADA.GOLESLOCAL) THEN 3
		WHEN LOCAL.ID = JORNADA.IDLOCAL AND (JORNADA.GOLESLOCAL < JORNADA.GOLESVISITAS) THEN 0
		ELSE 1
		END AS PUNTOSLOCAL,
	CASE 
	 	WHEN VISITAS.ID = JORNADA.IDVISITAS AND (JORNADA.GOLESVISITAS < JORNADA.GOLESLOCAL) THEN 0
		WHEN VISITAS.ID = JORNADA.IDVISITAS AND (JORNADA.GOLESLOCAL < JORNADA.GOLESVISITAS) THEN 3
		ELSE 1
		END AS PUNTOSVISITAS
	FROM JORNADA 
	INNER JOIN EQUIPOS AS VISITAS ON VISITAS.ID = JORNADA.IDVISITAS 
	INNER JOIN EQUIPOS AS LOCAL ON LOCAL.ID = JORNADA.IDLOCAL 
	INNER JOIN CANCHA ON CANCHA.ID = JORNADA.IDCANCHA WHERE VISITAS.ID = 2 OR LOCAL.ID=2;


    --- SUM ALL POINTS TABLERO
	...
	SUM(
		CASE 
			WHEN EQUIPOS.ID = JORNADA.IDVISITAS AND  
				(JORNADA.GOLESVISITAS < JORNADA.GOLESLOCAL) THEN 0
			WHEN  EQUIPOS.ID = JORNADA.IDVISITAS AND 
				(JORNADA.GOLESLOCAL < JORNADA.GOLESVISITAS) THEN 3
			WHEN  EQUIPOS.ID = JORNADA.IDVISITAS AND 
				(JORNADA.GOLESLOCAL = JORNADA.GOLESVISITAS) THEN 1
			ELSE 0 END
	) + 
	SUM(
		CASE 
			WHEN EQUIPOS.ID = JORNADA.IDLOCAL AND  
				(JORNADA.GOLESVISITAS < JORNADA.GOLESLOCAL) THEN 3
			WHEN  EQUIPOS.ID = JORNADA.IDLOCAL AND 
				(JORNADA.GOLESLOCAL < JORNADA.GOLESVISITAS) THEN 0
			WHEN  EQUIPOS.ID = JORNADA.IDLOCAL AND 
				(JORNADA.GOLESLOCAL = JORNADA.GOLESVISITAS) THEN 1
			ELSE 0 END
	) AS "VICTORIAS VISITAS"
